...
#include "Eds_BME680.h"  // Include the library header definition
...
struct 
	{ 
		...
		float         h_offset  = 1.5;                 // offset humidity sensor	
...
{
  return begin(I2C_STANDARD_MODE, 0, &Wire);  // Initialize I2C with standard speed, allow any i2c address, using i2c bus number given or standard Wire. (begin(NULL,NULL, &Wire1))
}  // of method begin()
bool BME680_Class::begin(const uint32_t i2cSpeed, TwoWire *theWire)
{
  /*!
  @brief   starts communications with device (overloaded)
  ...
  @param[in2] i2c bus BME680 is attached to.
  return "true" if successful otherwise false
  */
  return begin(i2cSpeed, 0, theWire);  // Initialize I2C with given speed, allow any i2c address
}  // of method begin()
...
*  @param[in] i2c bus BME680 is attached to.
*  return "true" if successful otherwise false
*/
bool BME680_Class::begin(const uint32_t i2cSpeed, const uint8_t i2cAddress, TwoWire *theWire)
{
    _wire = theWire;
    _wire->begin();             // Start I2C as master
    _wire->setClock(i2cSpeed);  // and set bus speed
    _I2CSpeed = i2cSpeed;
        ...
        {  // Loop through all I2C addresses
            if (i2cAddress == 0 || _I2CAddress == i2cAddress)
                {  // Only check if relevant
                    _wire->beginTransmission(_I2CAddress);               // Check for BME680 here
...
/*==================================================================================================================*/
/*End of begin overloaded methods section                                                                                   */
/*==================================================================================================================*/
...
//----------------------------------------------------------------------
void BME680_Class::set_offset_temp(float toffset) 
{  
...
//----------------------------------------------------------------------
float BME680_Class::get_offset_temp() 
{  
  return(param.t_offset); 
}

//----------------------------------------------------------------------
void BME680_Class::set_offset_hum(float hoffset) 
{
...
//----------------------------------------------------------------------
float BME680_Class::get_offset_hum() 
{  
  return(param.h_offset); 
}

//----------------------------------------------------------------------
bool   BME680_Class::isIAQValid()
{
 return(_isValidIaq); 
}
...
//----------------------------------------------------------------------
float BME680_Class::getAlt(void)
{
  //return (gdata.fAlt) ; 
...
//----------------------------------------------------------------------
float BME680_Class::getCalibAlt()
{
...
//----------------------------------------------------------------------
float BME680_Class::getSeaLevel()
{
...
//----------------------------------------------------------------------
float 	BME680_Class::getTVoc(void)
{
  return (gdata.fTvoc) ; 
}
...
/********************************************************************************************!
@brief   Returns the calculated Absolute Humidity from the input tempuratue, and Humidity.
param[in]  Temperature, Humidity in float format.
param[out] Absulute Humidity in float format.
*********************************************************************************************/
float BME680_Class::absHum(float temp, float hum)
{
    double sdd, dd;
...
/********************************************************************************************!
@brief   Returns the calculated Dew Point from the input tempuratue, and Humidity.
param[in]  Temperature, Humidity in float format.
param[out] Dew Point       Dew Point in float format.
*********************************************************************************************/
float BME680_Class::dewPoint(float temp, float hum)
{
...
/*!
@brief   Returns the most recent temperature, humidity, pressure, and gas readings.
...
uint8_t BME680_Class::getSensorData(int32_t& temp, int32_t& hum, int32_t& press, int32_t& gas, const bool waitSwitch)
{
    ...
    temp           = _Temperature + param.t_offset;     // Copy global variables to parameters
    hum            = _Humidity + param.h_offset;        // Copy global variables to parameters
...
{
    /*! Lookup table for the possible gas range values for integer method */
    ...
    /*! Lookup table for the possible gas range values for float method */
    /* This section is not implemented yet.
    const float lookupTable3[16] =
    {
        float(1), float(1), float(1), float(1), float(1), float(0.99), float(1), float(0.992),
        float(1), float(1), float(0.998), float(0.995), float(1), float(0.99), float(1), float(1)
    };
    */

    /* This section is not implemented yet.
    const float lookupTable4[16] =
    {
        float(8000000), float(4000000), float(2000000), float(1000000), float(499500.4995), float(248262.1648), float(12500), float(63004.03226),
        float(31281.28128), float(15625), float(7812.5), float(3906.25), float(1953.125), float(976.5625), float(488.28125), float(244.140625)
    };
    */
    ...
    //*******************************//
    // This computes the temperature using integers //
    //*******************************//
    ...
    //*******************************//
    // This computes the temperature using floating point //
    //*******************************//
    /*
    var1    = (((double)adc_temp / 16384.0) - ((double)_T1 / 1024.0)) * (double)_T2;
    var2    = ((((double)adc_temp / 131072.0) - ((double)_T1 / 8192.0)) * (((double)adc_temp / 131072.0) - ((double)_T1 / 8192.0))) * ((double)_T3 * 16.0);
    _tfine = var1 + var2;
    _Temperature = _tfine / 5120.0;
    */

    //*******************************//
    // This computes the pressure using integers     //
    //*******************************//
    ...
    //*******************************//
    // This computes the pressure using floating point //
    //*******************************//
    /*
    var1        = ((double)_tfine / 2.0) - 64000.0;
    var2        = var1 * var1 * ((double)_P6 / 131072.0);
    var2        = var2 + (var1 * (double)_P5 * 2.0);
    var2        = (var2 / 4.0) + ((double)_P4 * 65536.0);
    var1        = ((((double)_P3 * var1 * var1) / 16384.0) + ((double)_P2 * var1)) / 524288.0;
    var1        = (1.0 + (var1 / 32768.0)) * (double)_P1;
    _Pressure   = 1048576.0 - (double)adc_pres;
    _Pressure   = ((_Pressure - (var2 / 4096.0)) * 6250.0) / var1;
    var1        = ((double)_P9 * _Pressure * _Pressure) / 2147483648.0;
    var2        = _Pressure * ((double)_P8 / 32768.0);
    var3        = (_Pressure / 256.0) * (_Pressure / 256.0) * (_Pressure / 256.0) * (_P10 / 131072.0);
    _Pressure   = _Pressure + (var1 + var2 + var3 + ((double)_P7 * 128.0)) / 16.0;
    */


    //**********************//
    // Compute the humidity using integers //
    //**********************//
    ...
    //*******************************//
    // This computes the humidity using floating point //
    //*******************************//
    /*
    var1    = adc_hum - (((double)_H1 * 16.0) + (((double)_H3 / 2.0) * _Temperature));
    var2    = var1 * (((double)_H2 / 262144.0) * (1.0 + (((double)_H4 / 16384.0) * _Temperature) + (((double)_H5 / 1048576.0) * _Temperature * _Temperature)));
    var3    = (double)_H6 / 16384.0;
    var4    = (double)_H7 / 2097152.0;
    _Humidity   = var2 +((var3 +(var4 * _Temperature)) * var2 * var2);
    */

    //**********************//
    // Compute the Gas using integers     //
    //**********************//
    uint64_t uvar2;
    ...
    //*******************************//
    // This computes the Gas using floating point //
    //*******************************//
    /*
    var1    = ((double)_G1 / 16.0) + 49.0;
    var2    = (((double)_G2 / 32768.0) * 0.0005) + 0.00235;
    var3    = (double)_G3 / 1024.0;
    var4    = var1 * ( 1.0 + ( var2 * (double)GasTemp ) );
    var5    = var4 + (var3 * (double)_Temperature);
    res_heat_x  = (uint8_t)(3.4 * ((var5 * (4.0 / (4.0 + (double)_res_heat_range)) * (1.0 / (1.0 + ((double)_res_heat * 0.002)))) -25));

    var1    = (1340.0 + 5.0 * _rng_sw_err) * lookupTable3[gas_range];
    gas_res = var1 * lookupTable4[gas_range] / (adc_gas_res - 512.0 + var1);
...
{
  while (measuring()) {yield();}  // loop until any active measurment is complete
