...
class TMP117
{
  ...
  public:
              TMP117 (const uint8_t address, TwoWire *wire = &Wire);
    ...
    uint16_t  update (void);  // Returns the contents of the configuration register.
    void      softReset ( void );

    void      setAlertMode ( TMP117_PMODE mode);
    void      setCallBack( void (*_newDataCallback)(void) );  // Set the callback function address without setting any other options.
    void      setAlertCallback ( void (*allert_callback)(void), uint8_t pin );
    void      setAlertTemperature ( double lowtemp, double hightemp );
